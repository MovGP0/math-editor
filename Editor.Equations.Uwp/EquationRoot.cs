...
namespace Editor
{
    ...
    {
        private readonly Caret vCaret;
        private readonly Caret hCaret;
        ...
        {
            if (Keyboard.IsKeyDown(VirtualKey.LeftShift) || Keyboard.IsKeyDown(VirtualKey.RightShift))
            {
                IsSelecting = true;
                ActiveChild.StartSelection();
                ActiveChild.HandleMouseDrag(mousePoint);                
        ...
        {
            if ((Keyboard.IsKeyDown(VirtualKey.LeftShift) || Keyboard.IsKeyDown(VirtualKey.RightShift)) && (new[] { VirtualKey.Right, VirtualKey.Left, VirtualKey.Up, VirtualKey.Down, VirtualKey.Home, VirtualKey.End }).Contains(key))
            {
                if (!IsSelecting)
                {
                    IsSelecting = true;
            ...
            VirtualKey[] handledKeys = { VirtualKey.Left, VirtualKey.Right, VirtualKey.Delete, VirtualKey.Up, VirtualKey.Down, VirtualKey.Enter, VirtualKey.Escape, VirtualKey.Back, VirtualKey.Home, VirtualKey.End };
            bool result = false;
            if (handledKeys.Contains(key))
            {
                result = true;
                if (IsSelecting && (new[] { VirtualKey.Delete, VirtualKey.Enter, VirtualKey.Back }).Contains(key))
                {
                    ActiveChild.RemoveSelection(true);
                }
                else
