...
namespace Editor
{
    ...
    {
        readonly double lineSpaceFactor;
        ...
        private void DrawSelectionRegion(DrawingContext dc)
        {
            ...
            dc.DrawRectangle(Brush.LightGray, null, rect);
        ...
        public override bool Select(VirtualKey key)
        {
            ...
            {
                if ((Keyboard.IsKeyDown(VirtualKey.LeftControl) || Keyboard.IsKeyDown(VirtualKey.RightControl)) && SelectionStartIndex > 0)
                {
            ...
            {
                if ((Keyboard.IsKeyDown(VirtualKey.LeftCtrl) || Keyboard.IsKeyDown(VirtualKey.RightCtrl)) && SelectionStartIndex < childEquations.Count - 1)
                {
            ...
            else if (key == VirtualKey.Up && SelectionStartIndex >= 0 && childEquations.IndexOf(ActiveChild) > 0)
            {
            ...
            else if (key == VirtualKey.Down && SelectionStartIndex < childEquations.Count && childEquations.IndexOf(ActiveChild) < childEquations.Count - 1)
            {
                Point point = childEquations[SelectionStartIndex].GetVerticalCaretLocation();
                ActiveChild = childEquations[childEquations.IndexOf(ActiveChild) + 1];
                ((EquationRow)childEquations[SelectionStartIndex]).SelectToEnd();
        ...
        {
            if ((Keyboard.IsKeyDown(VirtualKey.LeftControl) || Keyboard.IsKeyDown(VirtualKey.RightControl)))
            {
            ...
            {
                if (key == VirtualKey.Up && ActiveChild != childEquations.First())
                {
                ...
                else if (key == VirtualKey.Down && ActiveChild != childEquations.Last())
                {
                    Point point = ActiveChild.GetVerticalCaretLocation();
                    ActiveChild = childEquations[childEquations.IndexOf(ActiveChild) + 1];
