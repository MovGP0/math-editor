    ...
    public class TextEquation : EquationBase, ISupportsUndo
    {
        ...
        public override CopyDataObject Copy(bool removeSelection)
        {
            ...
            if (selectedText.Length > 0)
            {
                ...
                {
                    dc.DrawRectangle(new SolidColorBrush(Colors.White), null, new Rect(0, 0, bitmap.Width, bitmap.Height));
                    DrawEquation(dc);
                }
                bitmap.Render(dv);
                int startIndex = SelectedItems > 0 ? SelectionStartIndex : SelectionStartIndex + SelectedItems;
        ...
        public override void Paste(XElement xElement)
        {
            if (xElement.Name.LocalName == GetType().Name)
            {
                ...
                {
                    int formatId = TextManager.GetFormatId(FontSize, fontType, FontStyle.Normal, FontWeights.Normal, new SolidColorBrush(Colors.Black), false);
                    for (int i = 0; i < text.Length; i++)
                    {
                        formats[i] = formatId;
            ...
            for (int i = startIndex; i < startIndex + count; i++)
            {
                switch (format.ToLower())
                {
                    ...
                    case "bold":
                        ...
                        break;
                    case "italic":
                        formats[i] = TextManager.GetFormatIdForNewStyle(formats[i], applied ? FontStyle.Italic : FontStyle.Normal);
                        break;
                    default:
        ...
        public override void SetCursorOnKeyUpDown(VirtualKey key, Point point)
        {
        ...
        protected override void CalculateHeight()
        {
            ...
            {
                int formatId = TextManager.GetFormatId(FontSize, fontType, InputItalic ? FontStyle.Italic : FontStyle.Normal, InputBold ? FontWeights.Bold : FontWeights.Normal, new SolidColorBrush(Colors.Black), InputUnderline);
        ...
        private sealed class HeightMetrics
        {
